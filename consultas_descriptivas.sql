...
SELECT DISTINCT 
	...
	s3.n_completo as 'Nombre completo representante legal',
	s3.cel as 'celular representante legal'
FROM CLIENTE c 
	...
	ON s3.id_scliente=sc.id_scliente
	LEFT JOIN (SELECT su.id_scliente, u.direc, u.cat
				FROM SE_UBICA su INNER JOIN UBICACION u ON u.cod_ubic=su.cod_ubic
...
-- ORDER BY t.cap DESC 
------------------------------------------------------------------------------------------
-- CANTIDAD DE TANQUES POR SUB CLIENTE CON CONTRATO VIGENTE
SELECT
	sc.id_scliente as 'Codigo de Cliente',
	sc.n_com as 'Nombre de Cliente',
	COUNT(t.cod_tk) as 'Cantidad total de tanques'
FROM SUB_CLIENTE sc
	INNER JOIN CONTRATO c ON sc.id_scliente=c.id_scliente
...
GROUP BY 
	sc.id_scliente,
	sc.n_com
ORDER BY COUNT(t.cod_tk) DESC 
------------------------------------------------------------------------------------------
-- CAPACIDAD M3 TOTAL POR SUB CLIENTE CON CONTRATO VIGENTE
SELECT 
	sc.id_scliente as 'Codigo de Cliente',
	sc.n_com as 'Nombre de Cliente',
	SUM(t.cap) as 'Capacidad total m3 de tanques'
FROM SUB_CLIENTE sc
	INNER JOIN CONTRATO c ON sc.id_scliente=c.id_scliente
	INNER JOIN EQUIPO e ON e.cod_con=c.cod_con
	INNER JOIN TANQUE t ON e.cod_equ=t.cod_tk
